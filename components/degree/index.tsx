...
const Degree = () => {
	...
	const handleDragStart = (event: DragStartEvent) => {
		...
		if (event.active.data.current?.children?.length > 0) {
			id = event.active.data.current!.children.filter(
		...
		const lowestIndex = req[event.active.id]
			? terms[
					[...req[event.active.id].map((r) => used[r])].sort(
						(a, b) => {
							if (a.year === b.year) {
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		if (over && over.id && active && active.id) {
			if (
				...
				const courseId = active.data.current?.children
					? active.data.current.children.find(
							(child: { selected: boolean }) => child.selected
					  ).course
					: active.id;
				...
				setUsed({
					...used,
					[courseId]: terms[(over.id as string).slice(0, 36)],
				});
				setMap(new Map(map));
			}
		}
							...
							style={{
								width:
									sideBar === SIDEBAR.NONE
										...
										: "73.5vw",
								position: "relative",
								right: sideBar !== SIDEBAR.INFO ? 0 : "25vw",
