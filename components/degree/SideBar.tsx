...
const SideBar = ({
	...
	useEffect(() => {
		if (!currentTerm || Object.keys(info).length === 0 || !currentTerm.id)
			return;
		const credits = (coursesMap.get(currentTerm.id) || []).reduce(
			(acc, curr) => acc + info[curr].credits,
			0
		);
		console.log(credits);
	...
	const handleDelete = async (courseCode: string, removeSupa?: boolean) => {
		...
		coursesMap.set(
			currentTerm.id,
			coursesMap.get(currentTerm.id)!.filter((c) => c !== courseCode)
		);
		setMap(coursesMap);
		setTerms({ ...terms });
	};
	...
	const getNextTerm = (type: TERMS, year: string) => {
		...
		if (
			type === "Fall" &&
			Object.values(terms).every(
				(t) => t.type !== "Spring" || t.year !== yearPlus
			)
		) {
	...
	const handleAddTerm = async (termType: TERMS, termYear: string) => {
		...
		const { id: termId, success } = await addTerm(
			id as string,
			term.year,
			term.type,
			supabase
				...
				Object.keys(temp)
					.sort((a, b) => {
						if (temp[a].year === temp[b].year) {
							...
							return (
								order.indexOf(temp[a].type) -
								order.indexOf(temp[b].type)
							);
						}
					...
					.reduce(
						(
							r: { [key: string]: { type: TERMS; year: string } },
							k
						) => ((r[k] = temp[k]), r),
						{}
					)
	...
	const handleDeleteTerm = async () => {
		if (
			currentTerm.type === majorMinor.term.type &&
			currentTerm.year === majorMinor.term.year
		) {
	...
	return (
		<div className="h-[calc(100vh-4rem)] absolute">
				...
				>
					...
					<div className="fixed inset-0 overflow-y-auto">
						<div className="flex min-h-full items-center justify-center p-4 text-center">
							...
							>
								<Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-light-green/30 backdrop-blur-md back pb-10 text-left align-middle shadow-xl transition-all">
									<Dialog.Title
										as="h3"
										className="text-2xl w-full font-med ium leading-6 text-white bg-green-pea p-4 font-Poppins font-bold"
									>
										Add term
									</Dialog.Title>
													...
													<Fragment key={index}>
														{next.map(
															(term, index) => (
																<div
																	key={index}
																	className="flex items-center justify-center p-2 py-3 font-Lato font-bold border bg-green-50 rounded cursor-pointer shadow hover:shadow-md transition-shadow"
																	onClick={() =>
																		handleAddTerm(
																			term.type,
																			term.year
																			...
																			"Fall" ? (
																			  ...
																			  "Spring" ? (
																				...
																				<LeafIcon
																					...
																					className="fill-green-500 mx-4 ml-2"
																				/>
																			)}
																		</div>
																		<p
			...
			<div
				className="h-full border border-dark-green/20 absolute z-[9999] resize-x"
				style={{
					right: show ? "75vw" : "98.5vw",
					transition: "all 0.3s ease-in-out",
					cursor: show ? "ew-resize" : "auto",
				}}
				onMouseEnter={() => {
					if (!show) setHovering(true);
				}}
				onMouseLeave={() => {
					if (!show || hovering) setHovering(false);
				}}
			></div>
			<div
			...
			>
				<div
					className="h-full w-[4vw] bg-green-pea flex flex-col"
					ref={onSideBarRefChange}
				>
					{Object.keys(terms).map((term, index) => {
						const eq =
							currentTerm.type === terms[term].type &&
							currentTerm.year === terms[term].year;
						return (
							<div
								key={index}
								className="h-14 w-full flex flex-col justify-center items-center border-r border-light-green/20"
							>
								<p
									onClick={() =>
										handleClick({
									...
									className={`font-Poppins font-extrabold text-center text-white border-cyan-700/60 w-f ull h-fu ll w-[85%] h-[85%] 
                                     ${
											terms[term].type +
											(eq
                                    ...
                                    rounded flex flex-col justify-center select-none`}
								>
									{map[terms[term].type] +
										terms[term].year.slice(2)}
								</p>
							</div>
						);
					})}
					<div className="h-14 w-full flex flex-col justify-center items-center">
						...
						<p
							onClick={openModal}
							className="font-Poppins text-xl font-extrabold text-center w-[90%] h-[90%] bg-dark-green text-white hover:brightness-90 rounded flex flex-col justify-center cursor-pointer select-none"
						>
							+
						</p>
					</div>
				...
				<div
					...
					// }}
				>
					<div className="w-full flex flex-row bg-green-pea items-center relative">
						<div className="w-full mx-auto h-24 border-0 border-light-green 0 flex flex-col justify-end items-center text-white font-bold font-Poppins text-2xl">
							<p>{currentTerm.type + " " + currentTerm.year}</p>
							<div className="flex flex-row font-JetBrainsMono pb-4">
								<p className="text-sm font-normal">
									{coursesMap.get(currentTerm.id)?.length}{" "}
									course
									{coursesMap.get(currentTerm.id)?.length ===
									1
										? ""
						...
						{coursesMap.get(currentTerm.id)?.length !== 0 ? (
							coursesMap
								...
								?.map((course, index) => {
									...
									const courseLevel =
										level[
											Math.max(parseInt(course[4]) - 1, 0)
										];
									return (
										<div
											key={index}
										...
										>
											<div
												className="border-2 border-black bg-cyan-100 rounded p-2 shadow-md font-JetBrainsMono"
												style={{
											...
											>
												<div className="w-full flex flex-row justify-between items-center">
													<p
														className="bg-cyan-700 text-sm font-extrabold w-fit rounded p-[1px] px-1 shadow-inner"
														style={{
															color: info[course]
																?.background,
														}}
													...
													<button
														...
														className="bg-red-60 0 text-xs hover:bg-red-700 hover:text-white transition-colors text-red-600 font-bold py-1 px-2 rounded"
													>
														x
													</button>
												</div>
												...
												<div className="mt-1 flex flex-row gap-2">
													<p
														className="text-black text-xs bg-black font-bold font-Lato w-fit rounded p-[1px] px-1"
														style={{
															backgroundColor:
																info[course]
																	?.background,
													...
													<p
														className={`text-white text-xs rounded p-[1px] px-1 bg${courseLevel?.color}`}
													>
														{courseLevel?.label}
													</p>
												</div>
					...
					<div className="bg-cyan-800 w-full h- 10 absolute bottom-0">
						<div className="bg-white rounded-bl h- 1">
