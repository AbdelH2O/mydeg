...
const SelectorNode: FC<NodeProps> = ({ data, dragHandle }) => {
	...
	const selected = (
		data.children as { course: string; selected: boolean; color: string }[]
	...
	const unlocked = !selected
		? false
		: selected &&
		  (req[selected.course] === undefined ||
				req[selected.course].every((r) => used[r] !== undefined));
	...
	const atrr =
		selected && sideBar === SIDEBAR.COURSES
	...
	return (
				...
				...{
					...
					backgroundColor:
						previouslyUsed && sideBar === SIDEBAR.COURSES
							...
							? selected.color
							: unlocked ||
							  (sideBar !== SIDEBAR.COURSES && selected)
							? data.background
		...
		>
			...
			>
				{selected
					? data.children.find(
							(child: { course: string; selected: boolean }) =>
								child.selected
					  )?.course
