...
const RightBar = () => {
    ...
    useEffect(() => {
        ...
        if(infoCourse.children) {
            ...
            setSelectedCourse(infoCourse.children.find((child) => child.selected) || {course: '', name: '', selected: false, color: ''});
            return;
        }
        setIndex(0);
    }, [infoCourse]);
    ...
    const selected = !infoCourse.children || infoCourse.children.some((child) => child.selected);
    ...
    const handleChange = async (e: {
        ...
        setEdges((edges) => {
            ...
            return [
                ...
                ...(req[e.course] ? req[e.course].map((requisite) => {
                    return {
        ...
        if(used[selectedCourse.course] !== undefined) {
            ...
            if(error) {
                ...
                setMap((map) => {
                    ...
                    const mapi = new Map(map.set(termId, courses.filter((course) => course !== selectedCourse.course)));
    ...
    return (
        <div className="h-[calc(100vh-4rem)] absolute">
            ...
            >
                    ...
                    <Tab.Group selectedIndex={index} onChange={setIndex}>
                        <div className="flex flex-row justify-center items-center px-4 py-3 transition-colors shadow-md font-Poppins bg-cyan-800">
                                <Tab.List className="flex flex-row space-x-4">
                                    <Tab disabled={!selected} className={({ selected }) => `text-md select-none h-full py-1 rounded px-3 ${selected ? 'font-bold text-white bg-cyan-900' : 'hover:bg-cyan-600/60 disabled:hover:bg-cyan-700 disabled:hover:text-cyan-100 disabled:cursor-not-allowed disabled:opacity-50 hover:text-cyan-50 transition-colors font-bold text-cyan-100'}`}>Course Info</Tab>
                                    {
                                        infoCourse?.children &&
                                        <Tab className={({ selected }) => `text-md select-none h-full py-1 rounded px-3 ${selected ? 'font-bold text-white bg-cyan-900' : 'hover:bg-cyan-600/60 hover:text-cyan-50 transition-colors font-bold text-cyan-100'}`}>Selection</Tab>
                                    }
                                ...
                                <Tab.Panels>
                                        {
                                            infoCourse.name || !infoCourse.children ? (
                                                <Tab.Panel>
                                                    <div className="bg-cyan-100 border border-cyan-200 rounded p-2">
                                                        <div className="text-xl font-bold font-Poppins">{infoCourse.name}</div>
                                                        <div className="text-sm font-bold text-cyan-500 font-JetBrainsMono">{infoCourse.id}</div>
                                                    </div>

                                                    <div className="flex flex-row space-x-2 mt-2">
                                                        {
                                                            Object.keys(level).indexOf(levelIndex) !== -1 ?
                                                            <div className={`text-xs font-bold text-white rounded px-2 py-1 bg${level[levelIndex].color}`}>{level[levelIndex].label}</div> :
                                                            <div className="text-xs font-bold text-white rounded px-2 py-1 bg-blue-600">Introductory</div>
                                                        }
                                                        <div className="text-xs font-bold text- rounded px-2 py-1 text-cyan-100 bg-cyan-600"><span className="">{infoCourse.credits}</span> credits</div>

                                                    </div>
                                                    <div className="mx-3 mt-2">
                                                        <div className="text-xl font-extrabold font-Lato my-2">Description:</div>
                                                        <div className="indent-4 text-sm font-Poppins">
                                                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed et nunc quam. Cras consectetur rutrum facilisis. Nam justo leo, placerat eget arcu eu, gravida elementum lorem. In hac habitasse platea dictumst. Sed et mi diam. Nam in tellus eu odio molestie aliquam faucibus vel justo. Duis ultricies sem eget lacus posuere, in volutpat libero iaculis. Aenean sollicitudin, dolor eu vulputate ullamcorper, lorem est hendrerit dui, sed elementum dolor neque eget libero. Nulla tincidunt quam a luctus feugiat. Sed aliquam lacus in egestas suscipit.
                                                        </div>
                                                    </div>
                                                </Tab.Panel>
                                            ) : (
                                                <>
                                                    <Tab.Panel>
                                                        <div className="bg-cyan-100 border border-cyan-200 rounded p-2">
                                                            <div className="text-xl font-bold font-Poppins">{selectedCourse.name}</div>
                                                            <div className="text-sm font-bold text-cyan-500 font-JetBrainsMono">{selectedCourse.course}</div>
                                                        </div>
                                                    ...
                                                    <Tab.Panel>
                                                        <div className="bg-cyan-100 border border-cyan-200 rounded p-2">
                                                            <div className="text-xl font-bold font-Poppins">{infoCourse.id}</div>
                                                            <div className="text-sm font-bold text-cyan-500 font-JetBrainsMono">{selectedCourse.name !== '' ? selectedCourse.name : "Select course"}</div>
                                                        </div>
                                                        ...
                                                        <input type="text" className="w-full rounded bg-white border mb-3 py-2 pl-10 shadow"></input>
                                                        <RadioGroup value={selectedCourse} onChange={handleChange}>
                                                            <RadioGroup.Label className="sr-only">Server size</RadioGroup.Label>
                                                            <div className="grid grid-cols-2 gap-4">
                                                                {infoCourse.children.map((child) => (
                ...
                <div className="w-[25vw] h-[calc(100vh-4rem)] bg-white z-[99999] absolute peer cursor-pointer" style={{display: !true ? 'block' : 'none', marginLeft: show ? '0' : '-23.5vw'}} onClick={() => {}}></div>
                <div
                ...
                >
                    <svg className="group-hover:fill-white peer-hover:fill-white fill-black" fill={!true ? 'black' : 'white'} 
