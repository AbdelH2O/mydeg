...
export const CoursesProvider = ({
	...
	const [terms, setTerms] = useState<{
		[key: string]: { type: TERMS; year: string };
	}>({});
	...
	useEffect(() => {
		if (id) {
			(async () => {
				...
				mm.term.type &&
					setCurrentTerm(
						mm.term as { type: TERMS; year: string; id: string }
					);
				if (mm.major) {
					const major = await getCourses(
						mm.major,
						id as string,
						setNodes,
						nodes,
...
const getDegree = async (
	id: string,
	setMajorMinor: React.Dispatch<
		React.SetStateAction<{
			major: string;
			minor: string;
			term: { type: TERMS; year: string };
		}>
	>,
	coursesMap: Map<string, string[]>,
	setTerms: React.Dispatch<
		React.SetStateAction<{ [key: string]: { type: TERMS; year: string } }>
	>,
	setUsed: React.Dispatch<
		React.SetStateAction<{ [key: string]: { type: string; year: string } }>
	>,
	...
	if (error) {
		...
		setMajorMinor({
			major: data[0].major,
			minor: data[0].minor,
			term: { type: indexMatch[data[0].start], year: data[0].year },
		});
		...
		if (
			...
			data[0].Degree_Term.forEach((term) => {
				terms[term.id] = { type: term.type as TERMS, year: term.year };
				if (
					term.Degree_Term_Course === null ||
				...
				const courses = term.Degree_Term_Course.map(
					(course: { term: string; course: string }) => {
				...
				Object.keys(terms)
					.sort((a, b) => {
						if (terms[a].year === terms[b].year) {
							...
							return (
								order.indexOf(terms[a].type) -
								order.indexOf(terms[b].type)
							);
						}
						return (
							parseInt(terms[a].year) - parseInt(terms[b].year)
						);
					})
					.reduce(
						(
...
const getCourses = async (
	...
	if (
		...
		data.length === 0 ||
		data[0] === null ||
		data[0].Major_Course === null ||
		!Array.isArray(data[0].Major_Course)
	) {
		throw error;
	}
	const nodes_pre = data[0].Major_Course.map((majorCourse) => {
		...
		if (majorCourse.parent !== null) {
			...
			if (
				...
				description =
					majorCourse.Course.description || "No description";
			}

			parents[majorCourse.parent!].push({
		...
		const data = {
			name:
				majorCourse.Course && !Array.isArray(majorCourse.Course)
					? majorCourse.Course.name
					: "",
			credits:
				majorCourse.Course && !Array.isArray(majorCourse.Course)
					? majorCourse.Course.credits
					: 0,
			desc:
				majorCourse.Course && !Array.isArray(majorCourse.Course)
					? majorCourse.Course.description
						? majorCourse.Course.description
						: "No description"
					: "",
			background: majorCourse.color,
	...
	const parentNodes: Node[] = Object.keys(parents).map((par: string) => {
		return {
			...
			data: {
				...
				description: parents[par] ? parents[par][0].desc : "",
			},
			position: {
				x: coord.get(parents[par][0].course)!.x * 100,
				y:
					(coord.get(parents[par][0].course)!.x % 2 !== 0
	...
	const { data: data2, error: error2 } = await supabase
		...
		.select(`*`)
		.in("course", [
			...data![0].Major_Course.map((majorCourse) => majorCourse.course),
			...selectables,
		]);
	...
	const edges_temp: Edge[] = data2
		...
		.map((requisite) => {
			...
			const name =
				isChild.get(requisite.course) !== undefined
					? isChild.get(requisite.course)!
					: requisite.course;
			return {
