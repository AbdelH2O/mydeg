...
const SideBar = ({
    ...
    const handleDeleteCourse: MouseEventHandler<HTMLDivElement> = async (e) => {
        ...
        setUsed(used);
        removeCourse(
            (e.target as HTMLElement).dataset.term!,
        ...
        map.set(
            (e.target as HTMLElement).dataset.term!,
            map
                .get((e.target as HTMLElement).dataset.term!)!
                .filter((c) => c !== (e.target as HTMLElement).innerText)
        );
        setMap(map);
        setTerms({ ...terms });
    };
    ...
    const addable = (index: number) => {
        ...
        req[activeId] &&
            req[activeId].forEach((requirement) => {
        ...
        return (
            ...
            req[activeId].every(
                (requirement) =>
                    used[requirement] !== undefined && used[requirement] < index
            )
        );
    };
    ...
    return (
        <div className="h-[calc(100vh-4rem)] absolute">
            ...
            >
                <div className="flex flex-row justify-center items-center select-none py-3 pb-0">
                    <div className="flex flex-row justify-start items-center bg-white align-middle h-fit rounded">
                        <p className="font-JetBrainsMono font-extrabold px-4 py-2 rounded text-2xl text-slate-900">
                            {major}
                        </p>
                    </div>
                    {/* <p className="font-extrabold mx-5">/</p> */}
                    <div className="flex flex-row justify-start items-center align-middle h-fit rounded">
                        <p className="font-JetBrainsMono font-extrabold px-4 py-2 rounded text-2xl text-slate-900">
                            {minor}
                        </p>
                    </div>
                ...
                <div
                    className="w-full h-full flex flex-col justify-start items-center pb-2 overflow-y-scroll"
                ...
                >
                    ...
                    <div className="flex-auto w-full">
                        {Object.keys(terms).map((term, index) => {
                            return (
                                <div
                                    className="min-h-[35vh] mx-4 rounded mt-4"
                                    key={term + index}
                                    style={
                                        {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <div
                                                            data-term={term}
                                                            className="text-white text-lg font-JetBrainsMono bg-black p-2 rounded h-fit flex justify-center items-center"
                                                        >
                                                            {course}
                                                        </div>
                                                    </div>
                        ...
                        {Object.keys(terms).length < 8 ? (
                            <div className="w-full h-16 px-4 mb-2">
                                <div
                                    ...
                                    className="w-full bg-cyan-100 border-cyan-300 border-dashed border h-full my-4 rounded flex justify-center items-center cursor-pointer hover:shadow-xl transition-shadow"
                                >
                                    <svg
